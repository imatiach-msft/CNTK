...
void TrainTruncatedLSTMAcousticModelClassifier(const DeviceDescriptor& device, bool testSaveAndReLoad)
{
    ...
    config = GetHTKMinibatchSourceConfig(baseFeaturesDim, numOutputClasses);
    minibatchSource = CreateCompositeMinibatchSource(config.SetTruncationLength(truncationLength));
