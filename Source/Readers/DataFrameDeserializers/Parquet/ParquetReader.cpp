...
namespace Microsoft { namespace MSR { namespace CNTK { namespace hdfs
{
    ...
    { 
        ...
        std::vector<std::vector<size_t>> rowCounts;
        ...
        for (int i = 0; i < sources.size(); i++)
        {
            ...
            if (!storedSchema)
            {
                ...
                numRowGroups = md -> num_row_groups();
                storedSchema = true;
            }

            std::vector<size_t> numRowsForRGs;
            // store the number of rows in the numRowsForRG vector for each rowgroup
            for (int rg = 0; rg < numRowGroups; rg++)
            {
                ...
                numRowsForRGs.push_back(numRowsInRG);
            }
            rowCounts.push_back(numRowsForRGs);
        }
